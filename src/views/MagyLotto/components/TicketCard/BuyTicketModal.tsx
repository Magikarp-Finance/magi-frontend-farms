		...
		<Modal title={TranslateString(450, 'Enter amount of tickets to buy')} onDismiss={onDismiss}>
			...
			<div>
				<Announce>
					{TranslateString(
						478,
						'Ticket purchases are final. Your GYA cannot be returned to you after buying tickets.'
					)}
				</Announce>
				<Final>{TranslateString(460, `You will spend: ${cakeCosts(val)} GYA`)}</Final>
			</div>
			<ModalActions>
				...
				<Button
					...
					fullWidth
					disabled={
						pendingTx || parseInt(val) > Number(maxTickets) || parseInt(val) > 50 || parseInt(val) < 1
					}
					onClick={async () => {
						setPendingTx(true)
						await handleBuy()
