...
const InfoBlock = styled.div`padding: 0 24px 24px;`
...
const NftCard: React.FC<NftCardProps> = ({ nft }) => {
	...
	const { bunnyId, name, previewImage, originalImage, description, collection, pathCollection } = nft
	...
	return (
		<Card isActive={walletOwnsNft}>
			<Image
				src={collection ? `/images/nfts/${pathCollection}/${previewImage}` : `/images/nfts/${previewImage}`}
				alt={name}
			...
			<CardBody>
				...
				walletOwnsNft && (
					<Button variant='danger' fullWidth onClick={onPresentBurnModal} mt='24px'>
						{TranslateString(999, 'Trade in for GYA')}
					</Button>
				)}
			</CardBody>
			<CardFooter p='0'>
